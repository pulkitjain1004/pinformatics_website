...
data data.mynsduh;
    set data.nsduh(keep=caseid cigever alcever cocever mjever txever medicare caidchip
        ...
        snfever chewever pipever crkever
...
array v{*} cigever alcever cocever mjever medicare caidchip champus prvhltin snfever chewever pipever crkever;
array bvar{*} bcigever balcever bcocever bmjever bmedicare bcaidchip bchampus bprvhltin bsnfever bchewever bpipever bcrkever;

* Add var for substance abuse to consider all (including the new added) variables;
subst_ext=0;
do i=1 to dim(bvar);
  if v{i}=1 then bvar{i}=1;
  else if v{i}=2 then bvar{i}=0;

  if bvar{i}=1 then subst_ext=1;
end;
